...
int RBInsert(long value,int counter,int base,__global int *node,__global int *LC,__global int *RC,__global int *P,__global int *C,__global int *L,__global int *R,__global int *TL){
		...
		if(!l){
			...
			while(C[P[counter]]){
				if( (P[counter] == LC[P[P[counter]]]) && L[counter]==0 && L[P[counter]]==0 && L[P[P[counter]]]==0 && L[RC[P[P[counter]]]]==0 ){
					...
					l2 = atomic_cmpxchg(&L[P[counter]],0,1);
					l3 = atomic_cmpxchg(&L[P[P[counter]]],0,1);
					l4 = atomic_cmpxchg(&L[LC[P[P[counter]]]],0,1);
...
int insertFixup(int index,__global int *LC,__global int *RC,__global int *P,__global int *C,__global int *L,__global int *R,__global int *TL, int base){
	
while(C[P[index]]){
		if( (P[index] == LC[P[P[index]]]) && L[index]==0 && L[P[index]]==0 && L[P[P[index]]]==0 && L[RC[P[P[index]]]]==0 ){
			__private int l1,l2,l3,l4;
			l1 = atomic_cmpxchg(&L[index],0,1);
			l2 = atomic_cmpxchg(&L[P[index]],0,1);
			l3 = atomic_cmpxchg(&L[P[P[index]]],0,1);
			l4 = atomic_cmpxchg(&L[RC[P[P[index]]]],0,1);
			//L[index]=1; L[P[index]]=1;  L[P[P[index]]]=1; L[RC[P[P[index]]]]=1;
			if(!l1 && !l2 && !l3 && !l4){
				__private int z = RC[P[P[index]]];
				if(C[z]){
					C[P[index]] = 0;
					C[z] = 0;
					C[P[P[index]]] = 1;
			...
			l2 = atomic_cmpxchg(&L[P[index]],0,1);
			l3 = atomic_cmpxchg(&L[P[P[index]]],0,1);
			l4 = atomic_cmpxchg(&L[LC[P[P[index]]]],0,1);
			//L[index]=1; L[P[index]]=1;  L[P[P[index]]]=1; L[LC[P[P[index]]]]=1;
			if(!l1 && !l2 && !l3 && !l4){
				__private int z = LC[P[P[index]]];
				if(C[z]){
					C[P[index]] = 0;
					C[z] = 0;
					C[P[P[index]]] = 1;
...
__kernel void rbtree(__global int* node, __global int* LC, __global int* RC, __global int* C, __global int* P, __global long * WL, 
														   __global int* L, __global int *counter, __global int *R, __global int *size,__global int *TL)
{
	...
	if(base < size[0]){
		...
		choice = RBInsert(WL[base],index,base,node,LC,RC,P,C,L,R,TL);
			if(choice == 2)
				WL[base] = -2;
			else if(choice == 0)
				WL[base] = -1;
...
__kernel void knl2(__global int* node, __global int* LC, __global int* RC, __global int* C, __global int* P, __global long * WL, 
	...
	if(base < size[0]){
		...
		if( WL[index] != -1){
			if(WL[index] >= 0){
				__private int choice;
				choice = RBInsert(WL[index],index,base,node,LC,RC,P,C,L,R,TL);
				if(choice == 2)
					WL[index] = -2;
				else if(choice == 0)
					WL[index] = -1;
				...
				__private int choice;
				choice = insertFixup(TL[index],LC,RC,P,C,L,R,TL,base);
				if(choice == 0)
					WL[index] = -1;
			}
		}
